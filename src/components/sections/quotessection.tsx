    ...
    <section className="max-w-[1380px] w-full mx-auto">
      ...
      <div className=" relative mt-10 mb-0">
        <div className=" grid grid-cols-4 gap-4 max-[400px]:grid-cols-1 max-[400px]:mx-6 max-[600px]:grid-cols-1 max-[600px]:mx-6">
          {quotes.map((item, i) => (
            <div key={i} className="quote-item flex-col  ">
              <h4 className="font-bold m-4 text-xl mb-2">{item.title}</h4>

              <p className="text-base ">{item.quote}</p>

              {item.href == "" ? (
                <div className="m-4 p-2 text-base bg-[none] text-[#0066f5] border-2 border-[#0066f5] cursor-pointer rounded-md flex justify-center items-center ">
                  <span className="font-bold">{item.button}</span>
                </div>
              ) : (
                <a
                  href={item?.href}
                  className="m-4 p-2 text-base bg-[none] text-[#0066f5] border-2 border-[#0066f5] cursor-pointer rounded-md flex justify-center items-center"
                >
                  <span className="font-bold">{item.button}</span>
                </a>
              )}
